#snitch_control_ellison_agn_lofr_proper_reffscale.py
import os
import time
import numpy 
import numpy as np
...
ellison_list = Table.read('/Users/smethurst/Projects/mangaagn/code/ellison/drpall-v2_0_1_z_mag_control_ellison.fits', format='fits')
...
if start > 0:
    ellison_list = Table.read('ellison_control_inferred_snitch_linear_interp.fits')
else:
    pass
...
for n in trange(start, len(ellison_list)):
    ...
    if os.path.isfile('../../data/manga-'+str(ellison_list['plateifu'][n])+'-LOGCUBE.fits.gz'):
        ...
        rbin = RadialBinning(par=RadialBinningPar(center=[0.0,0.0], pa=obsp['pa'], ell=obsp['ell'], radius_scale=obsp['reff'],
                                                  radii=[np.sort(r)[1]/obsp['reff'], -1, nbins], log_step=True))
        binid = rbin.bin_index(x,y)
        r, theta = rbin.sma_coo.polar(x, y)

        if len(np.unique(binid)) == nbins+1:
            pass
        else:
            rbin = RadialBinning(par=RadialBinningPar(center=[0.0,0.0], pa=obsp['pa'], ell=obsp['ell'], radius_scale=obsp['reff'],
                                                  radii=[np.sort(r)[2]/obsp['reff'], -1, nbins], log_step=True))
            binid = rbin.bin_index(x,y)
            r, theta = rbin.sma_coo.polar(x, y)
        ...
        for j in range(idms.shape[1]):
            ...
            if np.all(np.isnan(idms[0][j,:])):
                ...
                ellison_list['inferred tq'][n, j] = np.nan
                ellison_list['inferred tq plus error'][n, j] = np.nan
                ellison_list['inferred tq minus error'][n, j] = np.nan
        ...
        ellison_list.write('ellison_control_inferred_snitch_linear_interp.fits', format='fits', overwrite=True)
...
for k in range(len(labels)):
    ...
    try:
        plt.tight_layout()
        plt.savefig('figures/median_'+labels[k]+'_with_log_radius_ellison_control_findagetheninterp_nostillsf_notlastgyr.pdf', frameon=False, bbox_inches='tight', pad_inches=0.1, transparent=True)
    except(IndexError):
        pass
    plt.close('all')

#av_bins = np.append(np.append(np.linspace(0.01, 0.05, 2), np.linspace(0.1, 0.9, 5)), np.linspace(1, 2, 3))
# av_bins = 10**np.linspace(-2, 0.7, 15)
# av_i = np.empty((n+1, len(av_bins)))
# lims = [[0.001,14]]
...
#         except(ValueError):
#             pass
#     plt.plot(av_bins, 10**np.nanmedian(np.log10(av_i[:m+1,:]), axis=0), color='r', alpha=0.9)
#     plt.xlabel(r'$R/R_e$')
#     plt.ylabel(ax_labels[k])
#     plt.xscale('log')
#     plt.yscale('log')
...
#     try:
#         plt.tight_layout()
#         plt.savefig('figures/median_deltatq_with_log_radius_ellison_control_findagetheninterp_nostillsf_notlastgyr.pdf', frameon=False, bbox_inches='tight', pad_inches=0.1, transparent=True)
#     except(IndexError):
#         pass
#     plt.close('all')
...
#     try:
#         plt.tight_layout()
#         plt.savefig('figures/median_'+labels[k]+'_with_radius_ellison_control_WISE.pdf', frameon=False, bbox_inches='tight', pad_inches=0.1, transparent=True)
#     except(IndexError):
#         pass
#     plt.close('all')
