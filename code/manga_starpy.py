...
def walker_plot(samples, nwalkers, limit, truth, ID):
    ...
    #save_fig = './test/walkers_steps_'+str(ID)+'.pdf'
...
Z = (1.0- 0.79) * np.random.random_sample(25) + 0.79
#Z = np.array([0.79, 0.84, 0.89, 0.94, 0.99])
tq = np.linspace(1.0, 13.6, 5)
tau =  np.linspace(0.1, 3.5, 5)
...
for n in range(20, len(spec)):
    ...
    #     fig.savefig('./test/starpy_output_first_run_af_not_right_test_'+str(n)+'.png')

    #     fig = corner.corner(samples, labels=[r'$Z$', r'$t_q$', r'$\tau$'], truths=[k[n,0], k[n,1], k[n,2]], quantiles=([0.16, 0.5, 0.84]))    
    #     fig.savefig('./test/starpy_output_corner_first_run_af_not_right_test_'+str(n)+'.png')

    #     fig = walker_plot(samples, nwalkers, -1, k[n], n)
    #     fig.savefig('./test/walkers_steps_first_run_af_not_right_'+str(n)+'.pdf')

    #     burninload = np.load('./test/samples_burn_in_'+str(n)+'.npy')
    #     fig = walker_plot(burninload, nwalkers, -1, k[n], n)
    #     fig.savefig('./test/walkers_steps_burn_in_first_run_af_not_right_'+str(n)+'.pdf')
    ...
    # clustering pruning
    #samples = np.load('./test/samples_'+str(n)+'.npy')
    lnp = np.load('./test/lnprob_run_'+str(n)+'.npy').reshape(nwalkers, nsteps)
    ...
    fig.savefig('./test/starpy_output_test_pruning_'+str(n)+'.png')

    fig = corner.corner(samples, labels=[r'$Z$', r'$t_q$', r'$\tau$'], truths=[k[n,0], k[n,1], k[n,2]], quantiles=([0.16, 0.5, 0.84]))    
    fig.savefig('./test/starpy_output_corner_test_pruning_'+str(n)+'.png')

    fig = walker_plot(samples, nwalkers, -1, k[n], n)
    fig.savefig('./test/walkers_steps_pruning_'+str(n)+'.pdf')

    burninload = np.load('./test/samples_burn_in_'+str(n)+'.npy')
    fig = walker_plot(burninload, nwalkers, -1, k[n], n)
    fig.savefig('./test/walkers_steps_burn_in_wihtout_pruning_'+str(n)+'.pdf')
...
for j in range(1):
    ...
    for row in axes:
        for ax in row:
            im = mpimg.imread('./test/starpy_output_test_pruning_'+str(n)+'.png')
            place_image(ax, im)
            n+=1
    P.tight_layout()
    P.subplots_adjust(wspace=0.0)
    P.subplots_adjust(hspace=0.0)
    P.savefig('./test/mosaic_spec_starpy_test.pdf')
